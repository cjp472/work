// pages/curriculum_bianji_i/curriculum_bianji_i.js
var app = getApp();
// 手机的宽度
var windowWRPX = 750
// 拖动时候的 pageX
...
Page({
  ...
  inputnormalPrice: function (e) {
    this.data.inputnormalPrice = e.detail.value
  },
  inputnowPrice: function (e) {
    this.data.inputnowPrice = e.detail.value
  },

  go_i: function () {
    console.log("baocun");
    console.log(this.data.inputName);
    console.log(this.data.inputnormalPrice);
    console.log(this.data.inputnowPrice );
    ...
    }else if (this.data.imurl == '') {
      wx.showModal({
        title: '提示',
        content: '请选择封面图片',
      })
    } else if (!pattern.test(this.data.inputnormalPrice)){
      wx.showModal({
        title: '提示',
        content: '请正确输入原价',
      })
    ...
    } else if (!pattern.test(this.data.inputnowPrice)) {
      wx.showModal({
        title: '提示',
        content: '请正确输入现价',
      })
  ...
  gosave: function (arr) {
    ...
    wx.uploadFile({
      ...
      success: function (res) {
        ...
        // var that = this;
        wx.request({
          url: app.globalData.myUrl2 + 'minisoftware/tyHouTai/addProduct.do',
          data: {
            "khcode": app.globalData.khcode,
            "productName": that.data.inputName,
            "normalPrice": that.data.inputnormalPrice,
            "vipPrice": that.data.inputnowPrice,
            "imageOne":res.data,
